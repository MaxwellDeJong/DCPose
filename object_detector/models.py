...
class YOLOLayer(torch.nn.Module):
  ...
  def forward(self, x, targets=None):
    ...
    prediction = x.view(
      nB, nA, self.bbox_attrs, nG, nG).permute(0, 1, 3, 4, 2).contiguous()
    ...
    if targets is None:
      ...
      cls_acc = cls_acc.float().mean().item()

      inference_metrics =  {"loss": loss.item(),
                            "x": loss_x.item(),
                            "y": loss_y.item(),
                            "w": loss_w.item(),
                            "h": loss_h.item(),
...
class Darknet(torch.nn.Module):
  ...
  def save_weights(self, path, cutoff=-1):
    ...
    for module_def, module in zip(self.module_defs[:cutoff],
